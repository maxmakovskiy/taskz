...
public:
    ...
    AirportCounter(TIterator begin, TIterator end)
        : AirportCounter() // delegated constructor from C++11
    {
    ...
    {
        return database.at(static_cast<size_t>(airport));
    }

    void Insert(TAirport airport)
    {
        mapToArrayIndex(airport)++;
    }
    ...
    Items GetItems() const
    {
        ...
        {
            result.push_back(Item(TAirport(i), database[i]));
        }
...
private:
    static const size_t database_size = static_cast<size_t>(TAirport::Last_);
    std::array<size_t, database_size> database;
    
    size_t& mapToArrayIndex(TAirport airport)
    {
...
void TestManyConstructions() {
  ...
  for (auto& x : airports) {
      auto temp = gen_airport(rnd);
    x = static_cast<SmallCountryAirports>(temp);
  }
